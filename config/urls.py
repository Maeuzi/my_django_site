    ...
    <head>
        ...
        <style>
            body { 
                ...
                text-align: center; padding-top: 20px; margin: 0; overflow: hidden;
            }
            .container {
                background: white; border: 4px solid black; display: inline-block; 
                padding: 20px; box-shadow: 8px 8px 0px #000; position: relative; z-index: 10;
            }
            .grid { display: grid; grid-template-columns: repeat(4, 75px); gap: 10px; justify-content: center; margin-top: 20px; }
            .card { 
                width: 75px; height: 75px; border: 3px solid black; background: #ef5777; 
            ...
            .particle { position: absolute; pointer-events: none; font-size: 24px; z-index: 100; }
        </style>
    </head>
    <body>
        <div class="container">
        ...
        <script>
            ...
            function createExplosion(x, y) {
                for (let i = 0; i < 12; i++) {
                    ...
                    let posX = x; let posY = y; let opacity = 1;
                    const move = setInterval(() => {
                        ...
                        if (opacity <= 0) { clearInterval(move); p.remove(); }
                    }, 16);
                }
            }
                        ...
                        if (flipped.length === 2) {
                            if (flipped[0].innerText === flipped[1].innerText) {
                                setTimeout(() => {
                                    ...
                                    flipped = [];
                                    matches++;
                                    if(matches === 8) document.getElementById('msg').innerText = "SWEET VICTORY! 👑 ✨";
                                }, 400);
                            } else {
                                setTimeout(() => {
